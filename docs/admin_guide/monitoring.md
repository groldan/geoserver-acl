...
GeoServer ACL includes built-in monitoring capabilities through Spring Boot Actuator.
...
```
http://localhost:8080/acl/actuator/health
```
...
```
http://localhost:8080/acl/actuator/metrics
```

To view specific metrics:

```
http://localhost:8080/acl/actuator/metrics/http.server.requests
```
...
```
http://localhost:8080/acl/actuator/metrics/acl.rule.evaluation
http://localhost:8080/acl/actuator/metrics/http.server.requests
```
...
```
http://localhost:8080/acl/actuator/prometheus
```
...
```bash
curl -s http://localhost:8080/acl/actuator/health | grep -q '"status":"UP"' || send_alert
```
